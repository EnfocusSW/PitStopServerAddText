...
var PitStopServer = /** @class */ (function () {
    ...
    function PitStopServer(options) {
        ...
        this.createVariableSet = function (values) {
            ...
            _this.variableSet = path.join(_this.outputFolder, _this.variableSetName);
            _this.finalVariableSetPath = path.join(_this.outputFolder, _this.variableSetName);
            var xmlOptions = {
                header: true,
                indent: "  ",
            };
        ...
        this.updateConfigFile = function () {
            ...
            try {
                ...
                if (typeof _this.outputFolder == "string") {
                    ...
                    newText = xml.createTextNode(path.join(_this.outputFolder, _this.outputPDFName));
                ...
                if (_this.xmlReport == true) {
                    ...
                    var newReportPathText = xml.createTextNode(path.join(_this.outputFolder, _this.xmlReportName));
                ...
                if (_this.pdfReportType == "Regular" || _this.pdfReportType == "Annotated") {
                    _this.debugMessages.push("Defining the PDF report type as " + _this.pdfReportType);
                    var reportsNode = select("//cf:Reports", xml);
                    var newElemReportPDF = xml.createElement("cf:ReportPDF");
                    reportsNode[0].appendChild(newElemReportPDF);
                    var newElemReportPath = xml.createElement("cf:ReportPath");
                    var newReportPathText = xml.createTextNode(path.join(_this.outputFolder, _this.pdfReportName));
                    newElemReportPath.appendChild(newReportPathText);
                    newElemReportPDF.appendChild(newElemReportPath);
                    if (typeof _this.pdfReportTemplate == "string" && _this.pdfReportTemplate !== "") {
                        if (fs.existsSync(_this.pdfReportTemplate) == false) {
                            throw new Error("The report template file " + _this.pdfReportTemplate + " does not exist");
                        }
                        _this.debugMessages.push("Using the PDF report template " + _this.pdfReportTemplate);
                        var newElemReportTmplPDF = xml.createElement("cf:ReportTemplatePath");
                        newElemReportPDF.appendChild(newElemReportTmplPDF);
                        var newReportTmplPathText = xml.createTextNode(_this.pdfReportTemplate);
                        newElemReportTmplPDF.appendChild(newReportTmplPathText);
                    }
                }
                if (_this.taskReport == true) {
                    ...
                    var newReportPathText = xml.createTextNode(path.join(_this.outputFolder, _this.taskReportName));
         ...
         * Creates an XML string of a basic configuration file for PitStop Server
         */
        this.createAnnotatedReportTemplate = function () {
            _this.debugMessages.push("Creating an annotated report template");
            var prefsString = "#BEGINPREFS\n\n\"AnnotatedReportSettings\"\n#BEGINDICT\n\"BookmarksView\" \"ReportItemsView\"\n\"TransparencyColor\" \"0.00000000 10.00000000 0.00000000\"\n\"InitiallyOpenSummaryAnnotation\" \"true\"\n\"MustAddURI\" \"true\"\n\"ViewingOnly\" \"false\"\n\"SummaryAnnotationColor\" \"100.00000000 100.00000000 0.00000000\"\n\"AddSummaryAnnotation\" \"true\"\n#ENDDICT\n\n\"PitStopReport\"\n#BEGINDICT\n\"ReportLayoutType\" \"AnnotatedReport\"\n\"ReportLayoutVersion\" \"2\"\n\"ReportLayoutName\" \"Annotated Report\"\n\"ReportLayoutNamedeDE\" \"Kommentierter Bericht\"\n\"ReportLayoutNamefrFR\" \"Rapport annot\u00E9\"\n\"ReportLayoutNameesES\" \"Informe Anotado\"\n\"ReportLayoutNameitIT\" \"Rapporto Annotato\"\n\"ReportLayoutNamejaJA\" \"\u6CE8\u91C8\u4ED8\u304D\u30EC\u30DD\u30FC\u30C8\"\n\"ReportLayoutNamezhCN\" \"\u5E26\u6279\u6CE8\u7684\u62A5\u544A\"\n\"ReportLayoutNamenlNL\" \"Geannoteerd rapport\"\n\"ReportLayoutNameplPL\" \"Raport Adnotacji\"\n\"ReportLayoutNameptBR\" \"Relat\u00F3rio com anota\u00E7\u00F5es\"\n#ENDDICT\n\n#ENDPREFS\n";
            try {
                _this.debugMessages.push("Saving annotated report template " + _this.annotatedReportTemplatePath);
                fs.writeFileSync(_this.annotatedReportTemplatePath, prefsString, { encoding: "utf16le" });
            }
            catch (error) {
                throw error;
            }
        ...
        for (var option in options) {
            ...
            switch (option) {
                ...
                case "variableSet":
                    ...
                    break;
                case "pdfReportType":
                    this.pdfReportType = options.pdfReportType;
                case "pdfReportTemplate":
                    this.pdfReportTemplate = options.pdfReportTemplate;
                    break;
                case "pdfReportName":
        ...
        //create an annotated report template if needed and make it the PDF report template
        if (this.pdfReportType == "Annotated" && this.pdfReportTemplate == undefined) {
            this.annotatedReportTemplatePath = path.join(this.outputFolder, "AnnotatedReport.prefs");
            try {
                this.createAnnotatedReportTemplate();
            }
            catch (error) {
                throw error;
            }
            this.pdfReportTemplate = this.annotatedReportTemplatePath;
        }
        //create a basic config file if needed
        this.finalConfigFilePath = path.join(this.outputFolder, this.configFileName);
            ...
            try {
                ...
                for (var i = 0; i < enfocusList.length; i++) {
                    if (enfocusList[i].startsWith("Enfocus PitStop Server") == true) {
                        psPath = path.join(enfocusDir, enfocusList[i]);
                    }
                }
                if (psPath == "") {
    ...
    PitStopServer.findPitStopServerInRegistry = function () {
        ...
        if (applicationPath == "") {
            ...
            return applicationPath.replace("PitStop Server.exe", "PitStopServerCLI.exe"); //the CLI is in the same location
        }
    };
